...
const UploadDocuments = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Prepare an object to store the URLs of the documents
      const documentURLs = {};
          ...
          await new Promise((resolve, reject) => {
            uploadTask.on(
              ...
              async () => {
                ...
                documentURLs[field] = downloadURL; // Store the URL in the documentURLs object
                resolve();
              }
            );
          });
        ...
        {documentFields.map(({ label, field, icon }) => (
          <View key={field} style={styles.uploadCard}>
            ...
            <TouchableOpacity 
              style={[
                styles.uploadButton, 
                documents[field] ? styles.uploadButtonSuccess : null
              ]} 
              onPress={() => handlePdfUpload(field)}
              disabled={isSubmitting}
            >
              <Ionicons 
                name={documents[field] ? "checkmark-circle" : "cloud-upload-outline"} 
                size={22} 
                color="#fff" 
              />
              <Text style={styles.buttonText}>
                {documents[field] ? "Document Attached" : `Attach ${label}`}
              </Text>
            </TouchableOpacity>
